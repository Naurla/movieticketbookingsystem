...
public class MoviesDatabase {
	...
	public static void addNewMovie(Database database, Scanner s) {
		...
		String insert ="INSERT INTO `movies`(`ID`, `Name`, `Language`, `Genre`, "
                        + "`RunningTime`, `Starring`, `Rating`) VALUES ('"+ID+"','"+name+
...
//		
	...
	public static void updateMovie(Database database, Scanner s) {
		...
		System.out.println("Enter Movie RunningTime (-1 to keep old value): ");
		int runningTime = s.nextInt();
	...
	public static Movie getMovie(int ID, Database database) {
		...
		String select = "SELECT `ID`, `Name`, `Language`, `Genre`, `RunningTime`,"
	...
	public static ArrayList<Show> getAllMovieShows(Database database, int movieID) {
		...
		try {
			...
			while (rs.next()) {
				...
				String showTimeSt = rs.getString("showTime").trim();
	...
	public static int getNextShowID(Database database, int movieID) {
		int ID = 0;
